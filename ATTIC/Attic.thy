...
lemma inducts[consumes 1, case_names Nil Cons]:
  assumes \<open>f2 xs ys\<close>
      and \<open>P [] []\<close>
      and \<open>(\<And>x xs y ys. \<lbrakk>g2 x y; f2 xs ys; P xs ys\<rbrakk> \<Longrightarrow> P (x # xs) (y # ys))\<close>
...
lemmas map_upd_current = fun_upd_same[where y = \<open>Some _\<close>]
...
lemma butlast_list_of: "lfinite xs \<Longrightarrow> butlast (list_of xs) = list_of (lbutlast xs)"
  by simp
*)
lemma not_allI: "(\<exists>x. \<not>P x) \<Longrightarrow> \<not>(\<forall>x. P x)"
  by simp
...
lemma last:
  ...
  using assms f_eq by (metis Ball_set last_in_set)
...
lemma unzipL_no_tl:
  ...
  using map_eq_imp_length_eq[OF unzip] unzip by (induct vl\<^sub>1 vl\<^sub>2 rule: list_induct2, simp_all)
(*
lemma map_length_eq: \<open>length (map P list) = length (map Q list)\<close>
  by (induct list, auto)  *)
