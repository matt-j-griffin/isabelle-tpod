...
proposition unwind_trace:
  assumes unwind: \<open>unwind \<Delta>\<close> 
      ...
      and vn: \<open>validFromS\<^sub>o\<^sub>p\<^sub>t s\<^sub>1 tr\<^sub>1\<close> "never T\<^sub>o\<^sub>p\<^sub>t tr\<^sub>1" "completedFrom\<^sub>o\<^sub>p\<^sub>t s\<^sub>1 tr\<^sub>1" "tr\<^sub>1 \<noteq> []"
              \<open>validFromS\<^sub>o\<^sub>p\<^sub>t s\<^sub>2 tr\<^sub>2\<close> "never T\<^sub>o\<^sub>p\<^sub>t tr\<^sub>2" "completedFrom\<^sub>o\<^sub>p\<^sub>t s\<^sub>2 tr\<^sub>2" "tr\<^sub>2 \<noteq> []"
              \<open>validFromS\<^sub>v\<^sub>a\<^sub>n cs\<^sub>1 ctr\<^sub>1\<close> "never T\<^sub>v\<^sub>a\<^sub>n ctr\<^sub>1" "completedFrom\<^sub>v\<^sub>a\<^sub>n cs\<^sub>1 ctr\<^sub>1" "ctr\<^sub>1 \<noteq> []"
              \<open>validFromS\<^sub>v\<^sub>a\<^sub>n cs\<^sub>2 ctr\<^sub>2\<close> "never T\<^sub>v\<^sub>a\<^sub>n ctr\<^sub>2" "completedFrom\<^sub>v\<^sub>a\<^sub>n cs\<^sub>2 ctr\<^sub>2" "ctr\<^sub>2 \<noteq> []"
...
proof -
  ...
  using r vn S O \<Delta> proof(induction ctr\<^sub>1 ctr\<^sub>2 tr\<^sub>1 tr\<^sub>2 arbitrary: cs\<^sub>1 cs\<^sub>2 cvl\<^sub>1 cvl\<^sub>2 s\<^sub>1 s\<^sub>2 vl\<^sub>1 vl\<^sub>2 rule: measure_induct4[of ?f])
    ...
    note O = \<open>O\<^sub>v\<^sub>a\<^sub>n ctrr\<^sub>1 = O\<^sub>v\<^sub>a\<^sub>n ctrr\<^sub>2\<close> 
     ...
     and vnO1 = \<open>validFromS\<^sub>o\<^sub>p\<^sub>t s\<^sub>1 trr\<^sub>1\<close> \<open>never T\<^sub>o\<^sub>p\<^sub>t trr\<^sub>1\<close> \<open>completedFrom\<^sub>o\<^sub>p\<^sub>t s\<^sub>1 trr\<^sub>1\<close> \<open>trr\<^sub>1 \<noteq> []\<close>
     and vnO2 = \<open>validFromS\<^sub>o\<^sub>p\<^sub>t s\<^sub>2 trr\<^sub>2\<close> \<open>never T\<^sub>o\<^sub>p\<^sub>t trr\<^sub>2\<close> \<open>completedFrom\<^sub>o\<^sub>p\<^sub>t s\<^sub>2 trr\<^sub>2\<close> \<open>trr\<^sub>2 \<noteq> []\<close>
     and vn1 = \<open>validFromS\<^sub>v\<^sub>a\<^sub>n cs\<^sub>1 ctrr\<^sub>1\<close> \<open>never T\<^sub>v\<^sub>a\<^sub>n ctrr\<^sub>1\<close> \<open>completedFrom\<^sub>v\<^sub>a\<^sub>n cs\<^sub>1 ctrr\<^sub>1\<close> \<open>ctrr\<^sub>1 \<noteq> []\<close>
     and vn2 = \<open>validFromS\<^sub>v\<^sub>a\<^sub>n cs\<^sub>2 ctrr\<^sub>2\<close> \<open>never T\<^sub>v\<^sub>a\<^sub>n ctrr\<^sub>2\<close> \<open>completedFrom\<^sub>v\<^sub>a\<^sub>n cs\<^sub>2 ctrr\<^sub>2\<close> \<open>ctrr\<^sub>2 \<noteq> []\<close>
    ...
    unfolding proaction_def proof (elim disjE conjE[where P = \<open>finish _ _ _ _ _ _ _ _ _\<close>]
      ...
      moreover have \<open>Opt.\<psi> (\<Delta> cs\<^sub>1 cvl\<^sub>1 cs\<^sub>2 cvl\<^sub>2) trr\<^sub>1 trr\<^sub>2\<close>
        ...
        subgoal for tr\<^sub>1 tr\<^sub>2 s\<^sub>1 s\<^sub>2 vl\<^sub>1 vl\<^sub>2
          by (rule IH.IH[OF _ r(1,2) _ _ _ _ _ _ _ _ _ _ vn1 vn2 S(1,2) _ _ O,simplified, of _ _ _ s\<^sub>2])
        .
    ...
    next
      ...
      proof(cases \<open>length ctrr\<^sub>1 \<le> 1\<close>)
        ...
        hence ctrr\<^sub>1: \<open>ctrr\<^sub>1 = [cs\<^sub>1]\<close>
          using vn1(1,4) by (cases ctrr\<^sub>1, auto)
        ...
        have consume\<^sub>1: \<open>consume\<^sub>v\<^sub>a\<^sub>n cs\<^sub>1 cvl\<^sub>1 []\<close>
          ...
          by simp
        note noactionL = IH.IH[OF _ r(1) _ _ _ _ _ _ _ _ _ _ _ vn1 _ _ _ _ S(1),simplified] 
        show ?thesis
        proof(cases \<open>length ctrr\<^sub>2 \<le> 1\<close>)
          ...
          hence ctrr\<^sub>2: \<open>ctrr\<^sub>2 = [cs\<^sub>2]\<close>
            using vn2(1,4) by (cases ctrr\<^sub>2, auto)
          ...
          note noaction = noactionL[OF _ r(2) _ _ _ _ _ _ _ _ _ _ vn2 S(2) _ _ O, simplified]
        ...
        next
          ...
          hence nh2': "\<not> hopeless\<^sub>v\<^sub>a\<^sub>n cs\<^sub>2' cvl\<^sub>2'" 
            ...
            using tr2(1) ctrr\<^sub>2 tr2(2) by blast
          show ?thesis
          ...
          next
            ...
            note iactionR = noactionL[OF _ s2' _ _ _ _ _ _ _ _ _ _ tr2 _ Vtr1 _ _ O, unfolded ctrr\<^sub>2, simplified]
      ...
      next
        ...
        hence nh\<^sub>1': "\<not> hopeless\<^sub>v\<^sub>a\<^sub>n cs\<^sub>1' cvl\<^sub>1'" 
          unfolding hopeless_def apply auto
          using ctr\<^sub>1(1) ctrr\<^sub>1 ctr\<^sub>1(2) by blast      
        note actionL = IH.IH[OF _ cs\<^sub>1' _ _ _ _ _ _ _ _ _ _ _ ctr\<^sub>1 _ _ _ _ _ Sctr1, unfolded ctrr\<^sub>1, simplified] 
        show ?thesis
        proof(cases \<open>length ctrr\<^sub>2 \<le> 1\<close>)
          case True \<comment> \<open>No more right steps\<close>
          hence ctrr\<^sub>2: \<open>ctrr\<^sub>2 = [cs\<^sub>2]\<close>
          ...
          have consume\<^sub>2: \<open>consume\<^sub>v\<^sub>a\<^sub>n cs\<^sub>2 cvl\<^sub>2 []\<close>
            ...
            apply (drule Van.S_Cons_consume)
            by simp
          note iactionL = actionL[OF _ r(2) _ _ _ _ _ _ _ _ _ _ vn2 S(2)]
          show ?thesis 
          proof (cases \<open>isObs\<^sub>v\<^sub>a\<^sub>n cs\<^sub>1\<close>)
            case isObs1: True
          ...
          next
            ...
            note iactionL = actionL[OF _ r(2) _ _ _ _ _ _ _ _ _ _ vn2 S(2) _ _ O]
            hence \<psi>: \<open>Opt.\<psi> (\<Delta> cs\<^sub>1' cvl\<^sub>1' cs\<^sub>2 cvl\<^sub>2) trr\<^sub>1 trr\<^sub>2\<close>
              unfolding Opt.\<psi>_def apply (intro allI impI) 
              subgoal for _ _ s1 s\<^sub>2 vl\<^sub>1 vl\<^sub>2 
        ...
        next
          ...
          hence nh2': "\<not> hopeless\<^sub>v\<^sub>a\<^sub>n cs\<^sub>2' cvl\<^sub>2'" 
            unfolding hopeless_def apply auto
            using tr2(1,2) ctrr\<^sub>2 by blast
          thus ?thesis
          proof (cases \<open>isObs\<^sub>v\<^sub>a\<^sub>n cs\<^sub>1\<close>)
            case isObs1: True
            then show ?thesis 
            proof (cases \<open>isObs\<^sub>v\<^sub>a\<^sub>n cs\<^sub>2\<close>)
              ...
              hence uFor: \<open>Opt.unwindFor (\<Delta> cs\<^sub>1' cvl\<^sub>1' cs\<^sub>2' cvl\<^sub>2') s\<^sub>1 vl\<^sub>1 s\<^sub>2 vl\<^sub>2\<close>
                ...
                .
              note saction = actionL[OF _ s2' _ _ _ _ _ _ _ _ _ _ tr2 _ Vtr1 _ _ O'(2), unfolded ctrr\<^sub>2, simplified]
              have \<psi>: \<open>Opt.\<psi> (\<Delta> cs\<^sub>1' cvl\<^sub>1' cs\<^sub>2' cvl\<^sub>2') trr\<^sub>1 trr\<^sub>2\<close>
